...
inline float decibelToLinear(float dB) { return std::pow(10.0f, dB / 20.0f); }
...
auto peak_filter_init(peak_filter_t *filter, double f0, double q, double fs)
    -> void {
  filter->f0 = f0;
  filter->q = q;
  filter->fs = fs;
...
auto peak_filter_response(peak_filter_t *filter, double f)
    -> std::complex<double> {
