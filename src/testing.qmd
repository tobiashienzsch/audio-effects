...
```py
def relative_error(output1, output2, reference):
    return abs(output1 - output2) / abs(reference)
```
...
```py
def rms_error(output1, output2):
    return np.sqrt(np.mean((output1 - output2) ** 2))
```
...
```py
def psnr(output1, output2, reference):
    return 10 * np.log10(np.max(reference) ** 2 / np.mean((output1 - output2) ** 2))
```
